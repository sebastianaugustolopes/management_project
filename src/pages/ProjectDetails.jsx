...
export default function ProjectDetail() {
    ...
    const projectSignature = useMemo(() => {
        ...
        return {
            ...
            lastTaskId: tasks.length > 0 ? tasks[tasks.length - 1]?.id : null
        };
    }, [id, currentWorkspace]);
    
    const normalizedProject = useMemo(() => {
        if (!id || !currentWorkspace || !projectSignature) return null;
        
        const workspaceProjects = currentWorkspace.projects || [];
        if (workspaceProjects.length === 0) return null;
        
        const foundProject = workspaceProjects.find((p) => p.id === id);
        if (!foundProject) return null;
        
        // Normalize project: ensure tasks and members are arrays
        // Also ensure each task has all required fields
        const normalizedTasks = (Array.isArray(foundProject.tasks) ? foundProject.tasks : []).map(task => ({
            ...task,
            // Ensure assignee is properly structured
        ...
        return {
            ...
            members: Array.isArray(foundProject.members) ? foundProject.members : []
        };
