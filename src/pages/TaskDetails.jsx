...
import { deleteTask, fetchWorkspaces } from "../features/workspaceSlice";
...
const TaskDetails = () => {
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const comment = await commentAPI.create(taskId, {
    ...
    const handleDeleteTask = async () => {
        if (!taskId || !projectId) {
            toast.error("Task ID or Project ID is missing");
            return;
        }
        ...
        try {
            ...
            // Update Redux state
            dispatch(deleteTask({ id: taskId, projectId }));
                ...
                <div className="p-5 rounded-md bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 ">
                    <div className="mb-3">
                        <div className="flex items-start justify-between gap-2">
                            <h1 className="text-lg font-medium text-gray-900 dark:text-zinc-100 flex-1">{task.title}</h1>
                            <button
                ...
                {project && (
                    <div className="p-4 rounded-md bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-200 border border-gray-300 dark:border-zinc-800 ">
                        ...
                        {project.startDate || project.start_date ? (
                            <p className="text-xs mt-3">Project Start Date: {format(new Date(project.startDate || project.start_date), "dd MMM yyyy")}</p>
                        ) : null}
