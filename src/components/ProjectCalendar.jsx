...
const ProjectCalendar = ({ tasks = [] }) => {
    ...
    const getTasksForDate = (date) => safeTasks.filter((task) => {
    ...
    const upcomingTasks = safeTasks
        .filter((task) => {
            const dueDate = task.dueDate || task.due_date;
            return dueDate && !isBefore(new Date(dueDate), today) && task.status !== "DONE";
        })
        .sort((a, b) => {
            const dueDateA = a.dueDate || a.due_date;
            const dueDateB = b.dueDate || b.due_date;
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        {daysInMonth.map((day) => {
                            ...
                            const hasOverdue = dayTasks.some((t) => {
                                ...
                                return t.status !== "DONE" && dueDate && isBefore(new Date(dueDate), today);
                            });
                        ...
                        <div className="space-y-2">
                            {upcomingTasks.map((task) => (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-zinc-600 dark:text-zinc-400">{format(new Date(task.dueDate || task.due_date), "MMM d")}</p>
                                </div>
                            ))}
                        </div>
                    )}
